...
def retrieve_all_threads():
    try:
        ...
        print("Raw threads response:", data)  # Debug print
        
        # Filter out potential duplicates client-side
        unique_threads = []
        seen_ids = set()
        ...
        print(f"Found {len(unique_threads)} unique threads after filtering")
...
def load_full_conversation(thread_id):
    """Load complete conversation history"""
    try:
        response = requests.get(f"http://localhost:8000/threads/{thread_id}/full")
        response.raise_for_status()
...
def update_thread_title(thread_id, title):
    try:
        requests.put("http://localhost:8000/thread_title", 
...
if 'chat_threads' not in st.session_state:
    ...
    print(f"Frontend, Loaded chat threads: {st.session_state['chat_threads']}")
...
if user_input:
    ...
    try:
        ...
        with requests.post(
            ...
            # Create a new chat message container for the assistant
        ...
        if full_response:
            ...
            if len(st.session_state['message_history']) <= 2:
                update_thread_title(st.session_state['thread_id'], user_input[:30])
